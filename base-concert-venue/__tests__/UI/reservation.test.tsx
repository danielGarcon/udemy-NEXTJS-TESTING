...
test("reservation page shows correct number of available seats", async () => {
  render(<Reservation showId={0} seatCount={2} submitPurchase={jest.fn()} />);

  const seatCountText = await screen.findByText(/2 seats left/i);
...
test('reservation page shows "sold out" when no seats are available and NO purchase button', async () => {
  render(<Reservation showId={1} seatCount={0} submitPurchase={jest.fn()} />);
